# evpn-spine-leaf (L3) default config for PC4 (linux). Runs inside node netns (nsenter).
# Group2 tenant host on VLAN20/VNI10020 (leaf3 1/1/4 access). Same VNI/subnet as PC2,
# so PC2<->PC4 reach each other across the EVPN-VXLAN fabric (leaf2<->leaf3 VTEPs).
